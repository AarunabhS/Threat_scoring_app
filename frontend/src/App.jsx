...
function App() {
  ...
  const [selectedInsightCompany, setSelectedInsightCompany] = useState(competitorData[0]); 
  ...
  const tabDetails = {
    ...
    insights: {
      ...
      desc: "Comprehensive breakdown including raw scraped data and AI evaluations."
    },
    rankings: {
      title: "Leaderboard",
    ...
    <div className="min-h-screen text-slate-200 selection:bg-cyan-500/30 flex flex-col font-sans">
      ...
      <main className="w-full max-w-[1920px] mx-auto p-6 space-y-8 flex-1">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8 min-h-[calc(100vh-300px)]">
          <TabsList className="w-full max-w-3xl grid grid-cols-5 bg-slate-900/50 p-1 rounded-xl border border-white/5 mx-auto md:mx-0">
            {[
              { id: 'overview', icon: Activity, label: 'Overview' },
              { id: 'comparison', icon: BarChart3, label: 'Compare' },
              { id: 'radar', icon: PieChart, label: 'Profile' },
              { id: 'insights', icon: Cpu, label: 'Insights' },
                  ...
                  <CardContent className="flex-1 overflow-y-auto custom-scrollbar space-y-1">
                     {competitorData.map(c => (
                       <button
                         ...
                         className={`w-full flex items-center justify-between px-3 py-3 rounded-md text-sm transition-all border ${
                           compareSelection.includes(c.name)
          ...
          <TabsContent value="insights" className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full">
             <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-[800px]">
                <Card className="lg:col-span-4 glass-card h-full flex flex-col">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <Search className="w-5 h-5 text-cyan-400" />
                      Select Entity
                    </CardTitle>
                    <CardDescription>Select a company for detailed breakdown.</CardDescription>
                  </CardHeader>
                  <CardContent className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar max-h-[700px]">
                     {companiesWithOverall.map(company => (
                        <button
                          ...
                          className={`w-full text-left p-4 rounded-xl border transition-all duration-300 flex items-center justify-between group ${
                            selectedInsightCompany?.name === company.name 
                        ...
                        >
                           <div className="flex items-center gap-3">
                              ...
                              <div className="overflow-hidden">
                                 <div className={`font-semibold truncate transition-colors ${selectedInsightCompany?.name === company.name ? 'text-white' : 'text-slate-300'}`}>
                                   {company.name}
                                 </div>
                              </div>
                           </div>
                           <div className={`text-lg font-bold ${getScoreTextColor(company.overall)}`}>
                ...
                <div className="lg:col-span-8 space-y-6 h-full overflow-y-auto custom-scrollbar">
                          ...
                          <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 gap-4">
                               {Object.entries(selectedInsightCompany.raw).map(([key, value]) => {
                                 if (key === 'description') return null; // Skip description as it's in header
                                 return (
